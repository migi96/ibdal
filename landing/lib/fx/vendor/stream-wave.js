...
(function () {
  var ACCENT = [70, 198, 255], WHITE = [255, 255, 255], SKY = [150, 210, 255];
  ...
  function streamWave(canvas, opts) {
    ...
    var stop = ticker(canvas, function (dt, t) {
      ...
      /* soft body, rendered small and blurred, then blown up.
         opts.transparent skips the paper backdrop entirely so only the
         additive threads/streams below remain (overlay use on dark art). */
      ...
      if (!opts.transparent) ctx.drawImage(soft, px - 6, py - 4, W + 12, H + 8);
