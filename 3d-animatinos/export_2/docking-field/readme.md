...
New tiles, drawn in the artwork's own isometric footprint and palette, fly in along its four diagonals, dock around it, then release.
...
- `docking-field.js` — the animation. Exposes `window.TilesFX.dockingField`.
...
<script src="docking-field.js"></script>
<script>
  var opts = { speed: 1, intensity: 1, pointerFollow: true };
  TilesFX.ready()
    .then(function () { return TilesFX.loadImage('tiles.png'); })
    .then(function (img) {
      var anim = TilesFX.dockingField(document.getElementById('ai-tiles'), img, opts);
      // anim.destroy() on teardown; mutate opts at any time to retune live
    });
</script>
```

The library tags can be swapped for your own bundled copies of GSAP.
