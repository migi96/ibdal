...
Nodes run two tilted orbits around the disc, brightening as they come to the front, while a scan crosses the artwork and closes on a reticle when the pass completes.
...
- `scan-field.js` — the animation. Exposes `window.IconFX`.
...
<script src="scan-field.js"></script>
<script>
  var opts = { speed: 1, intensity: 1, pointerFollow: true };
  IconFX.ready()
    .then(function () { return IconFX.loadKit('icon.png'); })
    .then(function (kit) {
      var anim = IconFX.scanField(document.getElementById('ai-icon'), kit, opts);
...
The two orbits are drawn in code to sit with the rings already in the artwork — `tilt`, `rx` and `ry` in the orbit loop place them, and `NODES` sets how many run each ring.
