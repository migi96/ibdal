...
The four pieces draw in one after another and lock, with the seams flaring at the moment they meet, then ease apart again.
...
- `snap-together.js` — the animation. Exposes `window.IconFX`.
...
<script src="snap-together.js"></script>
<script>
  var opts = { speed: 1, intensity: 1, pointerFollow: true };
  IconFX.ready()
    .then(function () { return IconFX.loadKit('icon.png'); })
    .then(function (kit) {
      var anim = IconFX.snapTogether(document.getElementById('ai-icon'), kit, opts);
...
`splitQuadrants` cuts the artwork through its centre into a 2×2 grid. That slices the interlocking tabs, which is why `reach` (0.16 of the icon) keeps the separation small — raise it and the cut edges start to show.
