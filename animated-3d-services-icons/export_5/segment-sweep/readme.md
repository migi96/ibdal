...
The chart draws itself clockwise from twelve o'clock behind a moving edge, with a track underneath and the total settling above once the ring closes.
...
- `segment-sweep.js` — the animation. Exposes `window.IconFX`.
...
<script src="segment-sweep.js"></script>
<script>
  var opts = { speed: 1, intensity: 1, pointerFollow: true };
  IconFX.ready()
    .then(function () { return IconFX.loadKit('icon.png'); })
    .then(function (kit) {
      var anim = IconFX.segmentSweep(document.getElementById('ai-icon'), kit, opts);
...
The reveal is a wedge clip about the chart centre — `a0` is where it starts (twelve o'clock) and the sweep runs a full turn. The readout text and the `68%` figure are literals in the draw loop; swap them for your own.
