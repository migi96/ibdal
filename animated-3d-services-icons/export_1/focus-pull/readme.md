...
The assembly collapses along its own optical axis until the converging beam resolves to a point, holds in focus, then opens back out.
...
- `focus-pull.js` — the animation. Exposes `window.IconFX`.
...
<script src="focus-pull.js"></script>
<script>
  var opts = { speed: 1, intensity: 1, pointerFollow: true };
  IconFX.ready()
    .then(function () { return IconFX.loadKit('icon.png'); })
    .then(function (kit) {
      var anim = IconFX.focusPull(document.getElementById('ai-icon'), kit, opts);
...
The convergence is a horizontal scale about the focal point, so the elements draw together without ever needing to be separated. `fx` in the draw loop is that point — move it and the whole collapse re-origins.
